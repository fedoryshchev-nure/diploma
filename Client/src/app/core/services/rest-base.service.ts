...
export abstract class RestBaseService<TModel> {
	...
	post(model: TModel): Observable<TModel> {
		return this.http.post<TModel>(`${environment.apiUrl}/${this.controllerName}`, model);
	}

	put(id: string, model: TModel): Observable<TModel> {
		return this.http.post<TModel>(`${environment.apiUrl}/${this.controllerName}`, model, { params: { id } });
	}
